...
urls = ['https://finance.yahoo.com/quote/{ticker}/balance-sheet/',
...
# elements to find
# "button", class_="tabBtn l1 yf-gfq5ju", id_="tab-quarterly"
# "button", class_="tabBtn l1 yf-gfq5ju", id_="tab-annual"
# "div", class_="table yf-9ft13"
...
def get_financials(driver, url):
    ...
    #print(f"data_annual : {data_annual}")
    ...
    #print(f"data_quarterly : {data_quarterly}")
    
    # export content to text file
    #with open("fundamental_stock/content.txt", "w") as f:
    #    f.write(content)
...
try:
    ...
    for url in urls:
        ...
        df_annual, df_quarterly = get_financials(driver, url)
        #print(f"df_annual \n: {df_annual.head()}")
        #print(f"df_quarterly \n: {df_quarterly.head()}")

        # ALTERNATIVE: click to access other financial statements
        # default financials
        ...
        # click on button
        button_financials.click()
        time.sleep(2)
        print("clicked on button_financials")


        # save to csv
        df_annual.to_csv(f"fundamental_stock/scraper_csv/{ticker}_{financial_type}_annual.csv", index=False)
        df_quarterly.to_csv(f"fundamental_stock/scraper_csv/{ticker}_{financial_type}_quarterly.csv", index=False)
